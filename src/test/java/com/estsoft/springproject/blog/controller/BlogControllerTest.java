...
class BlogControllerTest {
    ...
    public void findOne() throws Exception {
        ...
        // then: API 호출 결과 검증 -> (given절에서 추가한 데이터가 그대로 json의 형태로 넘어오는지)
        resultActions.andExpect(status().isOk())
    ...
    // 단건 조회 API - id에 해당하는 자원이 없을 경우 (실패시) (4xx) 예외처리 검증
    @Test
    public void findOneException() throws Exception {
        // when: API 호출
        ResultActions resultActions = mockMvc.perform(get("/articles{id}", 1L)
                .accept(MediaType.APPLICATION_JSON));
        // then: Exception 검증, resultActions SATUS CODE 검증
        resultActions.andExpect(status().isBadRequest());

        org.junit.jupiter.api.Assertions.assertThrows(IllegalAccessException.class,() -> blogService.findBy(1L));

    }

    // todo 블로그 글 삭제 API 호출 테스트
    // 글 정보를 insert하고 삭제 API 호출(STATUS CODE), 검증 repository.findAll()
    @Test
    public void deleteTest() throws Exception {
        Article article = repository.save(new Article("blog title", "blog content"));
        Long id = article.getId();

        ResultActions resultActions = mockMvc.perform(delete("/article/{id}", id));
    ...
    // PUT /article/{id} body(json content) 요청
    @Test
    ...
    // 수정API 호출시 예외 발생했을 경우(수정하려는 id 존재하지 않음) = status code 4xx 검증, Exception 검증
    @Test
    public void updateArticleException() throws Exception {
        ...
        org.junit.jupiter.api.Assertions
                .assertThrows(IllegalAccessException.class,() -> blogService.update(notExistsId, request));
    }
}